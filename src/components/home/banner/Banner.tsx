...
const Banner = () => {
  ...
  return (
    <div className="relative  md:h-[350px] lg:h-[400px] h-64 ">
      {images.map((ele, idx) => (
        <div key={idx} className={`${getClass(idx)} w-full h-full`}>
          <div className="bg-blue-950 rounded-2xl   relative overflow-hidden h-full w-full">
            <div className="relative z-10 w-full h-full flex  py-6 px-4 lg:px-8 ">
      ...
      <div className="bg-gray-50 lg:p-2 absolute xl:-left-6 left-1 top-1/2 -translate-y-1/2 z-50 rounded-full opacity-35 lg:opacity-100">
        <button
          onClick={goPrev}
          className="  bg-blue-50 hover:bg-blue-100 text-white p-2 lg:p-3 rounded-full  cursor-pointer"
        >
          <GrPrevious size={16} color="#2563EB" />
        </button>
      </div>
      <div className="bg-gray-50 absolute  xl:-right-6 right-1 top-1/2 -translate-y-1/2 lg:p-2 z-50 rounded-full opacity-35 lg:opacity-100">
        <button
          onClick={goNext}
          className=" bg-blue-50 hover:bg-blue-100 text-white p-2 lg:p-3 rounded-full z-50 cursor-pointer"
        >

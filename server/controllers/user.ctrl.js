...
exports.updateUserCtrl = async (req, res, next) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(
      ...
      { new: true }
    ).select("-password -createdAt -updaedAt -wishlist -compareProducts -cart");
...
exports.addWishlistCtrl = async (req, res, next) => {
  ...
  try {
    ...
    if (checkWishlist) {
      ...
      return res.status(200).json({ type: "removed" });
    } else {
      user = await User.findByIdAndUpdate(
        userId,
        {
      ...
      return res.status(200).json({ type: "added" });
    }
  } catch (error) {
    next(error);
  }
...
//Get wish list
exports.getWishlistProductsForUserCtrl = async (req, res, next) => {
  try {
    const findUser = await User.findById(req.user.id)
      .select({
        wishlist: 1,

...
exports.deleteCouponCtrl = async (req, res, next) => {
  try {
    ...
    if (!findCoupon) {
      return next(new ApiHandler(400, "Coupon not found"));
    }
...
exports.applyCouponCtrl = async (req, res, next) => {
  try {
    ...
    console.log(coupon.expiry.getTime() < new Date().getTime());
    if (coupon.expiry.getTime() < new Date().getTime()) {
      return next(new ApiHandler(400, "Coupon has expired"));
    }
